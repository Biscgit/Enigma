...
class Rotor():
    ...
    def add_offset(self, char: chr, back: bool) -> chr:
        ...
        """
        value = (self.get_ord(char, back) + (self.start if back else -self.start)) % Rotor.len_al
        return self.mapped_alphabet[value] if back else Rotor.alphabet[value]

    def rotate_offset_scramble(self, char: chr, rotate: bool, back: bool) -> Tuple[bool, chr]:
        """
    ...
    def get_ord(self, char: chr, back: bool) -> int:
        ...
        """
        return self.mapped_alphabet.index(char.lower()) if back else Rotor.alphabet.index(char.lower())
